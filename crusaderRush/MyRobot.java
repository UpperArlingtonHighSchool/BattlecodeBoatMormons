...
public class MyRobot extends BCAbstractRobot {
	...
	private Action castle()
	{
		...
		if(me.turn > 1 && me.turn < 7)
		{
			if(hRefl)
			{
				return buildUnit(SPECS.CRUSADER, (me.x < fullMap.length / 2) ? 1 : -1, me.turn % 3 - 1);
			}
			else
			{
				return buildUnit(SPECS.CRUSADER, me.turn % 3 - 1, (me.x < fullMap.length / 2) ? 1 : -1);
			}
		}
	...
	private Action crusader()
	{
		...
		if(me.turn == 1)
		{
			...
			if(hRefl)
			{
				return move((me.x < fullMap.length / 2) ? 2 : -2, 0);
			}
			else
			{
				return move(0, (me.x < fullMap.length / 2) ? 2 : -2);
			}
		...
		else
		{
			...
			if(atk == null)
			{
				...
				if(hRefl)
				{
					return move((me.x < fullMap.length / 2) ? 1 : -1, 0);
				}
				else
				{
					return move(0, (me.x < fullMap.length / 2) ? 1 : -1);
				}
			}
			else
			{
